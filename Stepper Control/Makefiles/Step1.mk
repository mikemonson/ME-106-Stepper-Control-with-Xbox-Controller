...
#
LINKIT_1    = $(PACKAGES_PATH)/LinkIt

ifneq ($(wildcard $(LINKIT_1)),)
    LINKIT_APP      = $(LINKIT_1)
    LINKIT_PATH     = $(LINKIT_APP)
    LINKIT_BOARDS   = $(LINKIT_PATH)/hardware/arm/$(LINKIT_ONE_RELEASE)/boards.txt
endif
# ~~
...
# ~
        # More boards
        else ifneq ($(call PARSE_FILE,$(BOARD_TAG),name,$(LINKIT_BOARDS)),)
            include $(MAKEFILE_PATH)/LinkItOne_165.mk

        else ifneq ($(call PARSE_FILE,$(BOARD_TAG),name,$(LIGHTBLUE_PATH)/hardware/LightBlue-Bean/avr/boards.txt),)
        ...
        else ifneq ($(call PARSE_FILE,$(BOARD_TAG),name,$(EDISON_YOCTO_PATH)/boards.txt),)
            include $(MAKEFILE_PATH)/IntelYocto.mk
        else ifneq ($(call PARSE_FILE,$(BOARD_TAG),name,$(EDISON_MCU_PATH)/boards.txt),)
